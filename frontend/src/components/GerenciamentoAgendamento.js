...
function GerenciamentoAgendamento({titulo, subtitulo, botoes, estados, setEstados, visualizacao, mode}){
    ...
    return (
        <Acao titulo={titulo} subtitulo={subtitulo} botoes={botoes} campos={[
            ...
            {component: <TextField visualizacao={visualizacao} setValue={setStatus} value={status} placeholder="Selecione o cliente do agendamento" titulo="Status do agendamento"  tipo={visualizacao ? "input" : "select"} id="status" dadosSelect={[{"cod": 1, 'status':'Pendente'}, {"cod": 2, 'status':'Cancelado'}, {"cod": 3, 'status':'Concluído'}]}/>, buttonSupport: false}
        ]} />
    )
}
